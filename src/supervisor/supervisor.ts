...
export class Supervisor {
  ...
  async onOpenCodeEvent(event: OpenCodeEvent): Promise<SupervisorEffects> {
    ...
    // Multi-own (migration v12): a session may be a member of N swarms (its
    // coordinator rows plus, in edge cases, worker rows). A session event
    // applies to EVERY member row with that session, and the wake effects must
    // include every member so mail in EACH swarm delivers. For the common
    // worker case this is exactly one row — identical behavior to before.
    const members = await this.store.listMembersBySessionId(sessionID);
    if (members.length === 0) return effects;
